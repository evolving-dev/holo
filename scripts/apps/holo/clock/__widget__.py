...
if not (widget["date"] == widget["texts_old"][0] and widget["time"] == widget["texts_old"][1]):
    ...
    #Render the texts
    widget["date_surface"] = FONTS["p-sans-serif"].render(widget["weekday"] + ", " + widget["date"], True, STATIC_CORE["text_color"])
    widget["time_surface"] = FONTS["h4"].render(widget["time"], True, STATIC_CORE["text_color"])
